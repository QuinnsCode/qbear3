// app/pages/user/CreateLairClient.tsx
"use client";
...
// Tier configuration (same as BetterAuthSignup)
const TIERS = {
  free: {
    name: 'Free',
    icon: '🏕️',
...
interface CreateLairClientProps {
  userId: string;
  userName: string;
  userEmail: string;
}

export default function CreateLairClient({ userId, userName, userEmail }: CreateLairClientProps) {
  ...
  useEffect(() => {
    if (!lairName && isHydrated) {
      const displayName = userName || userEmail?.split('@')[0] || 'Player';
      setLairName(`${displayName}'s Lair`);
    }
  }, [userName, userEmail, lairName, isHydrated]);
  ...
  const handleCreateLair = async () => {
    try {
      console.log('Creating lair for existing user:', userId);
      setResult("");
      ...
      console.log('✅ Lair created:', result);
      ...
      if (selectedTier !== 'free') {
        ...
        const checkoutUrl = `https://qntbr.lemonsqueezy.com/checkout/buy/${variantId}?checkout[email]=${encodeURIComponent(userEmail)}&checkout[custom][user_id]=${userId}`;
      ...
      // Free tier - redirect to subdomain
      setResult("Lair created! Redirecting to your lair...");
      setTimeout(() => {
        window.location.href = result.redirectUrl;
      }, 1500);

    } catch (error) {
      console.error('Create lair error:', error);
          ...
          <div className="w-full max-w-md">
            ...
            <div className="text-center mb-8">
              ...
              <FantasyTitle size="lg" className="mb-3">
                Welcome, {userName?.split(' ')[0] || 'Adventurer'}!
              </FantasyTitle>
              <FantasyText variant="primary" className="text-base lg:text-lg">
                Let's create your lair to get started
              </FantasyText>
            </div>
