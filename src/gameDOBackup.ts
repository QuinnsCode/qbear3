...
//         // 🎯 NEW: Initialize bidding properties
...
//   // ✅ CLEANED UP - Uses extracted handleSetupPhaseProgression
...
//     } else if (this.gameState.status === 'playing') {
//       // ✅ Handle transition to main game
//       console.log(`🎮 Game transitioned to main game - starting main game AI if needed`)
//       const currentPlayer = this.gameState.players[this.gameState.currentPlayerIndex]
//       if (currentPlayer && globalAIController.isAIPlayer(currentPlayer.id)) {
//         console.log(`🤖 Scheduling MAIN GAME AI turn for ${currentPlayer.name} in Phase ${this.gameState.currentPhase}`)
        
//         const timeoutId = setTimeout(() => {
//           this.doAIMainGameAction()
//         }, this.AI_TURN_SPEED_MS * 3) // Longer delay for main game transition
...
//     // Step 1: Calculate and collect energy
//     const territoryCount = currentPlayer.territories.length
//     const energyIncome = Math.max(3, Math.floor(territoryCount / 3)) // Minimum 3, 1 per 3 territories
...
//       // Step 2: Place units (same amount as energy)
//       const unitsToPlace = energyIncome
//       const playerTerritories = currentPlayer.territories.filter(tId => 
//         this.gameState?.territories[tId]?.ownerId === currentPlayer.id
//       )
      
//       let unitsPlaced = 0
//       const placeNextUnit = async () => {
//         if (unitsPlaced >= unitsToPlace || !this.gameState) {
//           // All units placed, advance phase
...
//         // Pick random territory to place unit
...
//         // Place next unit after delay
//         setTimeout(placeNextUnit, this.AI_TURN_SPEED_MS)
//       }
      
//       // Start placing units after energy collection
//       setTimeout(placeNextUnit, this.AI_TURN_SPEED_MS)
...
//   private advancePlayerPhase(gameState: GameState): GameState {
//     const newState = { ...gameState }
...
//         return this.advancePlayerPhase(gameState)
